...
from parameters import TAIWAN_50

### Constant Variable ###
RAW = "../raw_data"
DATASET = "../Dataset/Universe/Taiwan_50"
...
def find_valid_interval(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.loc[df["identifier"].isin(TAIWAN_50)]
    start_date = max(
        [df.loc[df["identifier"] == id][["date_"]].min().values[0] for id in TAIWAN_50]
    )
    tw_50 = df.loc[df["date_"] >= start_date]
...
def save_prices_file(
    ...
    with tqdm(total=table.shape[0]) as pbar:
        for index, data in tqdm(table.iterrows()):
            ...
            if len(filtered_df) == len(TAIWAN_50):
...
def main():
    ...
    mapping = mapping.loc[
        (mapping["ticker"].isin(TAIWAN_50)) & (mapping["exchange"] == "TW")
    ].reset_index()
